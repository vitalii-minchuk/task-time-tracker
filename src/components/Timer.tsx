...
const Timer: FC<TimerProps> = ({ timer, setMessage }) => {
  ...
  return (
    <div className="animate-appear first:relative z-30 h-9 sm:h-12 p-1 m-3 mb-5 flex 
    justify-between  gap-1 sm:gap-6 items-center border-2 rounded-full border-slate-400
    opacity-90 hover:bg-slate-300"
    >
      <p className={`${isPlaying ? "text-green-600" : "text-gray-900"} pl-2 text-xs 
      sm:text-base`}
      >
        {timer.title}
      </p>
      <div className={`flex text-xs sm:text-lg ml-auto 
      ${isPlaying ? "text-green-600" : "text-gray-900"}`}
      >
